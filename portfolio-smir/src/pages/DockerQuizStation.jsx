// src/pages/DockerQuizStation.jsx
import React, { useState, useEffect, useRef, useCallback } from "react";
import { useNavigate } from "react-router-dom";
import { 
  Zap, Brain, Code2, Trophy, Clock 
} from "lucide-react";
import { DOCKER_QUESTIONS, DIFFICULTY_META } from "../components/stations/docker/dockerQuestions";
...
import { dockerQuizHistory } from "../components/stations/shared/quizHistory";
...
const buildShuffledSet = (level) => {
  const base = DOCKER_QUESTIONS[level] || [];
...
const renderWithCode = (text) => {
  ...
  return parts.map((part, idx) =>
    idx % 2 === 1 ? (
      <code
        key={idx}
        className="px-1 py-0.5 rounded border border-slate-700/80 bg-slate-900/90 font-mono text-[0.85em] text-sky-200"
      >
        {part}
      </code>
    ) : (
...
const GradientBorder = ({ children, color = "from-sky-500" }) => (
  <div
    className={`relative rounded-2xl overflow-hidden bg-gradient-to-br ${color} to-cyan-500 p-0.5`}
  >
...
const ModuleHeader = ({ icon: Icon, title, subtitle, color }) => (
  <div className="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
    <div className="flex items-center gap-3">
      ...
      <div>
        <h2 className="text-2xl font-bold text-transparent bg-gradient-to-r from-sky-300 via-cyan-300 to-blue-300 bg-clip-text">
          {title}
        </h2>
        {subtitle && (
  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-xs">
        ...
        <span className="font-bold text-sky-300">{pct}%</span>
      </div>
      <div className="w-full h-3 overflow-hidden border rounded-full bg-slate-900 border-slate-800">
        <div
          className="h-full transition-all duration-500 rounded-full bg-gradient-to-r from-sky-400 to-cyan-400"
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
...
export default function DockerQuizStation() {
  ...
  const handleNext = () => {
    ...
    if (nextIndex >= total) {
      ...
      dockerQuizHistory.addResult(result);
      loadStats();
    } else {
  ...
  return (
    <div className="relative min-h-[100vh] w-full px-4 py-6 sm:px-6 sm:py-8">
      <div className="absolute inset-0 pointer-events-none opacity-60">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,#0ea5e922_0,#02061700_55%,#020617_100%)]" />
      </div>
      ...
      <div className="relative z-10 max-w-4xl mx-auto space-y-4">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <ModuleHeader
            icon={Code2}
            title="Station Docker 🐳 – Quiz Galactique"
            subtitle="Teste tes connaissances Docker et progresse niveau par niveau."
          ...
          <div className="flex gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 px-3 py-2 text-xs font-semibold border shadow-sm rounded-xl border-sky-500/40 bg-slate-950/70 text-sky-100 shadow-sky-500/20 backdrop-blur-md hover:border-sky-300 sm:text-sm"
            >
        ...
        {!finished && (
          <GradientBorder color="from-sky-500">
                    ...
                    <button
                      ...
                      className={`inline-flex flex-1 items-center justify-center gap-1 rounded-lg px-3 py-2 text-xs font-semibold transition-all sm:text-sm ${
                        active
                          ...
                          : "border border-slate-700/80 bg-slate-900/70 text-slate-300 hover:border-sky-500/40"
                      }`}
                    >
                  ...
                  <div className="grid gap-2">
                    {currentQuestion.choices.map((choice, idx) => {
                      ...
                      if (isCorrectChoice) {
                        ...
                        stateClasses =
                          "border-sky-500/80 bg-sky-500/20 text-sky-100 ring-1 ring-sky-400/60";
                      }
                  ...
                  <div className="flex flex-col gap-2 pt-1 sm:flex-row sm:justify-between">
                    ...
                    <button
                      ...
                      className={`rounded-lg px-4 py-2 text-xs font-semibold sm:text-sm ${
                        isAnswered
                          ? "bg-gradient-to-r from-sky-500 to-cyan-500 text-white shadow-md shadow-sky-500/30 hover:brightness-110"
