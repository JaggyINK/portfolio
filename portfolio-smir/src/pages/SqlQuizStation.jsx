// src/pages/SqlQuizStation.jsx
import React, { useState, useEffect, useRef, useCallback } from "react";
import { useNavigate } from "react-router-dom";
import { 
  Zap, Brain, Code2, Trophy, Clock 
} from "lucide-react";
import { SQL_QUESTIONS, DIFFICULTY_META } from "../components/stations/sql/sqlQuestions";
...
import { sqlQuizHistory } from "../components/stations/shared/quizHistory";
...
const buildShuffledSet = (level) => {
  const base = SQL_QUESTIONS[level] || [];
...
const renderWithCode = (text) => {
  ...
  return parts.map((part, idx) =>
    idx % 2 === 1 ? (
      <code
        key={idx}
        className="px-1 py-0.5 rounded border border-slate-700/80 bg-slate-900/90 font-mono text-[0.85em] text-blue-200"
      >
        {part}
      </code>
    ) : (
...
const GradientBorder = ({ children, color = "from-blue-500" }) => (
  <div
...
const ModuleHeader = ({ icon: Icon, title, subtitle, color }) => (
  <div className="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
    <div className="flex items-center gap-3">
      ...
      <div>
        <h2 className="text-2xl font-bold text-transparent bg-gradient-to-r from-blue-300 via-cyan-300 to-sky-300 bg-clip-text">
          {title}
        </h2>
        {subtitle && (
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="w-full h-3 overflow-hidden border rounded-full bg-slate-900 border-slate-800">
        <div
          className="h-full transition-all duration-500 rounded-full bg-gradient-to-r from-blue-400 to-cyan-400"
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
...
export default function SqlQuizStation() {
  ...
  const loadStats = useCallback(() => {
    setGlobalStats(sqlQuizHistory.getGlobalStats());
  }, []);
  ...
  const handleNext = () => {
    ...
    if (nextIndex >= total) {
      ...
      sqlQuizHistory.addResult(result);
      loadStats();
    } else {
  ...
  return (
    <div className="relative min-h-[100vh] w-full px-4 py-6 sm:px-6 sm:py-8">
      <div className="absolute inset-0 pointer-events-none opacity-60">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,#3b82f622_0,#02061700_55%,#020617_100%)]" />
      </div>
      ...
      <div className="relative z-10 max-w-4xl mx-auto space-y-4">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <ModuleHeader
            icon={Code2}
            title="Station SQL – Quiz Galactique"
            subtitle="Teste tes connaissances SQL et progresse niveau par niveau."
          ...
          <div className="flex gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 px-3 py-2 text-xs font-semibold text-blue-100 border shadow-sm rounded-xl border-blue-500/40 bg-slate-950/70 shadow-blue-500/20 backdrop-blur-md hover:border-blue-300 sm:text-sm"
            >
                    ...
                    <button
                      ...
                      className={`inline-flex flex-1 items-center justify-center gap-1 rounded-lg px-3 py-2 text-xs font-semibold transition-all sm:text-sm ${
                        active
                          ...
                          : "border border-slate-700/80 bg-slate-900/70 text-slate-300 hover:border-blue-500/40"
                      }`}
                    >
                  ...
                  <div className="grid gap-2">
                    {currentQuestion.choices.map((choice, idx) => {
                      ...
                      if (isCorrectChoice) {
                        ...
                        stateClasses =
                          "border-blue-500/80 bg-blue-500/20 text-blue-100 ring-1 ring-blue-400/60";
                      }
                  ...
                  <div className="flex flex-col gap-2 pt-1 sm:flex-row sm:justify-between">
                    ...
                    <button
                      ...
                      className={`rounded-lg px-4 py-2 text-xs font-semibold sm:text-sm ${
                        isAnswered
                          ? "bg-gradient-to-r from-blue-500 to-cyan-500 text-white shadow-md shadow-blue-500/30 hover:brightness-110"
